function Department() {
    ...
    this.initDepartmentsDatatable = function () {
        $('#departments_datatable').DataTable({
            ...
            "ajax": {
                "type": "POST",
                "url": application.url+'/company/departments/data',
                "data": function ( d ) {
                    d.status = $('#status').val();
    ...
    this.initDepartmentSave = function () {
        application.onSubmit('#company_department_create_update_form', function (result) {
            application.showLoader('company_department_create_update_form_button');
            application.post('/company/departments/save', '#company_department_create_update_form', function (res) {
                ...
                if (result.success === 'true') {
                    ...
                    application.hideLoader('company_department_create_update_form_button');
                    application.showMessages(result.messages, 'company_department_create_update_form');
                }
            });
        });
    };
    ...
    this.initDepartmentCreateOrEditForm = function () {
        ...
        $('.create-or-edit-department').on('click', function () {
            ...
            application.load('/company/departments/create-or-edit'+id, modal+' .modal-body-container', function (result) {
    ...
    this.initDepartmentChangeStatus = function () {
        ...
        $('.change-department-status').on('click', function () {
            ...
            application.load('/company/departments/status/'+id+'/'+status, '', function (result) {
    ...
    this.initDepartmentDelete = function () {
        ...
        $('.delete-department').on('click', function () {
            ...
            var id = $(this).data('id');
            if (status === true) {
                application.load('/company/departments/delete/'+id, '', function (result) {
                    self.initDepartmentsDatatable();
                });
            }
        });
    ...
    this.initDepartmentsListBulkActions = function () {
        ...
        $('.bulk-action').on('click', function (e) {
            ...
            if (ids.length === 0) {
                ...
                application.post('/company/departments/bulk-action', {ids:ids, action: $(this).data('action')}, function (result) {
                    $('.bulk-action').val('');
