function JobFilter() {
    ...
    this.initJobFiltersDatatable = function () {
        $('#job_filters_datatable').DataTable({
            "aaSorting": [[ 8, 'desc' ]],
            "columnDefs": [{"orderable": false, "targets": [0,2,10]}],
            ...
            "ajax": {
                "type": "POST",
                "url": application.url+'/company/job-filters/data',
                "data": function ( d ) {
                    d.status = $('#status').val();
    ...
    this.initJobFilterUpdateValuesForm = function () {
        ...
        $('.add-job-filter-values').on('click', function () {
            ...
            application.load('/company/job-filters/update-values/'+id, modal+' .modal-body-container', function (result) {
    ...
    this.initJobFilterAddValue = function () {
        ...
        $('.add-value').on('click', function () {
            application.load('/company/job-filters/new-value', '', function (result) {
    ...
    this.initJobFilterValuesSave = function () {
        application.onSubmit('#company_job_filter_values_update_form', function (result) {
            application.showLoader('company_job_filter_values_update_form_button');
            application.post('/company/job-filters/update-values', '#company_job_filter_values_update_form', function (res) {
                var result = JSON.parse(application.response);
                if (result.success === 'true') {
                    $('#modal-default').modal('hide');
                    self.initJobFiltersDatatable();
                } else {
                    application.hideLoader('company_job_filter_values_update_form_button');
                    application.showMessages(result.messages, 'company_job_filter_values_update_form');
                }
            });
        });
    };
    ...
    this.initJobFilterCreateOrEditForm = function () {
        ...
        $('.create-or-edit-job-filter').on('click', function () {
            ...
            application.load('/company/job-filters/create-or-edit'+id, modal+' .modal-body-container', function (result) {
    ...
    this.initJobFilterSave = function () {
        application.onSubmit('#company_job_filter_create_update_form', function (result) {
            application.showLoader('company_job_filter_create_update_form_button');
            application.post('/company/job-filters/save', '#company_job_filter_create_update_form', function (res) {
                var result = JSON.parse(application.response);
                if (result.success === 'true') {
                    $('#modal-default').modal('hide');
                    self.initJobFiltersDatatable();
                } else {
                    application.hideLoader('company_job_filter_create_update_form_button');
                    application.showMessages(result.messages, 'company_job_filter_create_update_form');
                }
            });
        });
    };
    ...
    this.initJobFilterChangeStatus = function () {
        ...
        $('.change-job-filter-status').on('click', function () {
            ...
            application.load('/company/job-filters/status/'+id+'/'+status, '', function (result) {
    ...
    this.initJobFilterDelete = function () {
        ...
        $('.delete-job-filter').on('click', function () {
            ...
            var id = $(this).data('id');
            if (status === true) {
                application.load('/company/job-filters/delete/'+id, '', function (result) {
                    self.initJobFiltersDatatable();
                });
            }
        });
