<form id="company_job_filter_create_update_form">
    <input type="hidden" name="job_filter_id" value="{{ encode($job_filter['job_filter_id']) }}" />
    <div class="modal-body">
        <div class="row">
            ...
            <div class="col-md-6">
                <div class="form-group">
                    <label>{{ __('message.admin_filter') }}</label>
                    <select class="form-control" name="admin_filter">
                    <option value="1" {{ sel($job_filter['admin_filter'], 1) }}>{{ __('message.yes') }}</option>
                    <option value="0" {{ sel($job_filter['admin_filter'], 0) }}>{{ __('message.no') }}</option>
                    </select>
                </div>
            </div>
            <div class="col-md-6">
    ...
    <div class="modal-footer">
        ...
        <button type="submit" class="btn btn-primary btn-blue" id="company_job_filter_create_update_form_button">
        {{ __('message.save') }}
        </button>
    </div>
</form>