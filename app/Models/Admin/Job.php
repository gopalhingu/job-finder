...
class Job  extends Model
{
    ...
    public static function storeJob($data, $edit = null)
    {
        ...
        //Removing variables
        unset($data['traites'], $data['jobTags'], $data['quizes'], $data['labels'], $data['values'], $data['custom_field_ids'], $data['filters'], $data['_token'], $data['job_id']);
    ...
    public static function insertTraites($traites, $job_id, $employer_id)
    {
        //First deleting
    ...
    public static function insertQuizes($quizes, $job_id, $employer_id)
    {
        //First deleting
    ...
    public static function insertCustomFields($customFields, $job_id, $employer_id)
    {
    ...
    public static function assignJobFilterValues($filters, $job_id, $employer_id)
    {
        $all_value_ids = array();
        foreach ($filters as $job_filter_id => $job_filter_value_ids) {
            ...
            foreach ($job_filter_value_ids as $job_filter_value_id) {
                ...
                $existing_filter = Self::checkExistingJobFilter($job_filter_value_id, $job_id, $employer_id);
    ...
    private static function checkExistingJobFilter($job_filter_value_id, $job_id, $employer_id)
    {
    ...
    public static function remove($job_id)
    {
        ...
        //Tenth remove job filter value assignments
        DB::table('job_filter_value_assignments')->where(array('job_id' => $job_id))->delete();
    }
    ...
    public static function jobsList($request)
    {
        ...
        $query->select(
            ...
            'departments.title as department',
            'employers.company',
            DB::Raw('CONCAT('.dbprfx().'employers.first_name, '.dbprfx().'employers.last_name) as employer'),
        ...
        if (isset($request['employer_id']) && $request['employer_id'] != '') {
            $query->where('jobs.employer_id', $request['employer_id']);
        }
        $combined = array();
    ...
    public static function bulkAction($data)
    {
        ...
        switch ($action) {
            case "activate":
                Self::whereIn('job_id', $ids)->update(array('status' => '1'));
            break;
            case "deactivate":
                Self::whereIn('job_id', $ids)->update(array('status' => '0'));
            break;
        }
    }
