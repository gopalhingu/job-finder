...
class JobFiltersController extends Controller
{
    ...
    public function listView()
    {
        ...
        return view('company.job-filters.list', $data);
    }
    ...
    public function createOrEdit($job_filter_id = NULL)
    {
        ...
        echo view('company.job-filters.create-or-edit', compact('job_filter'))->render();
    }
    ...
    public function save(Request $request)
    {
        ...
        // $this->checkActiveJobFilters($edit, $request->input('status'));
        ...
        if (JobFilter::valueExist('title', $request->input('title'), $edit)) {
            echo json_encode(array(
                'success' => 'false',
                'messages' => $this->ajaxErrorMessage(array('error' => __('message.job_filter_already_exists')))
            ));
        }
    ...
    public function updateValuesForm($job_filter_id = NULL)
    {
        ...
        echo view('company.job-filters.values', compact('values', 'job_filter_id'))->render();
    }
    ...
    {
        echo view('company.job-filters.value')->render();
    }
     ...
     * @return html
     */
    private function checkActiveJobFilters($id = '', $status = '')
    {
        if ($id != '' && $status == '0') {
            return false;
        }
        ...
        $totalActiveFilters = JobFilter::getTotalJobFilters($id);
        $totalAllowedActiveFilters = empMembership(companyId(), 'active_custom_filters');
        if ($totalAllowedActiveFilters == '-1') {
            return false;
        }
        if ($totalActiveFilters >= $totalAllowedActiveFilters) {
            $detail = '<br />'.__('message.current_active').' : '.$totalActiveFilters;
            $detail .= '<br />'.__('message.allowed_in_membership').' : '.$totalAllowedActiveFilters.'<br />';
            die(json_encode(array(
                'success' => 'false',
                'messages' => $this->ajaxErrorMessage(array('error' => __('message.active_job_filters_limit_message').$detail))
            )));
        }        
    }    
}
