...
class DepartmentsController extends Controller
{
    ...
    public function createOrEdit($department_id = NULL)
    {
        ...
        if ($department['employer_id'] == 0) {
            $department['slug'] = 'admin';
        }
        //dd($department);
        echo view('admin.departments.create-or-edit', compact('department'))->render();
    }
    ...
    public function save(Request $request)
    {
        ...
        $department = Department::getDepartment('departments.department_id', $edit);
        if ($edit && $request->input('image')) {
            $this->deleteOldFile($department['image']);
        }   

        $emp_path = config('constants.upload_dirs.employers');
        $path = !empty($department['employer_id']) ? $emp_path.$department['slug'] : $emp_path.'admin';

        //Uploading new file
        $fileUpload = $this->uploadPublicFile(
            $request, 'image', $path.'/'.config('constants.upload_dirs.departments'), 
    ...
    public function delete($department_id)
    {
        ...
        $department = Department::getDepartment('departments.department_id', $department_id);
        $this->deleteOldFile($department['image']);
